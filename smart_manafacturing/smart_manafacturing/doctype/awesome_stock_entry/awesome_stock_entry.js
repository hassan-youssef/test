...
frappe.ui.form.on('Awesome Stock Entry', {
	...
	onload(frm) {
	  ...
	  frm.events.item_ref(frm)
	  refresh_field("stock_entries");
	  frm.get_field("stock_entries").$wrapper.html('<div class = "results"></div');
	  frm.doc.styles = {
		Draft: "red",
		Stopped: "red",
		"Not Started": "red",
	...
	item_ref:function(frm){
	  if (!frm.doc.item_ref)
		return
	  frappe.call({
		method:"smart_manafacturing.smart_manafacturing.doctype.awesome_stock_entry.awesome_stock_entry.get_available_dates",
		args:{
		...
		callback:function(r){
		  ...
		  if (options.length > 0) {
			frm.doc.stock_entry_date = options[0]
		  } else {
			frm.doc.stock_entry_date = ''
		  }
		  frm.set_df_property("stock_entry_date", "options", options);
		  frm.refresh_field('stock_entry_date')
	...
	set_table_html: function (frm) {
	  ...
	  if (!frm.doc.stock_entry_date && !frm.doc.batch_no) {
		frappe.throw(__("Please Set Stock Entry Date "));
	  }
	  frappe.call({
		method:
		  "smart_manafacturing.smart_manafacturing.doctype.awesome_stock_entry.awesome_stock_entry.get_stock_entries_by_ref",
		args: {
		  item_ref: frm.doc.item_ref,
		  stock_entry_date: frm.doc.stock_entry_date,
		...
		callback: function (r) {
			...
			r.message.x.forEach((x) => {
			  ...
			  r.message.y.forEach((y) => {
				...
				/*html+= `<td>
						  ...
						  `*/
				let disabled = !obj.stock_entry ? "disabled" : "";
				// let finish_disabled = !obj.has_finish ? "disabled" : "";
				let finish_disabled = !obj.has_finish ? "disabled" : "";
				let start_disabled = !obj.has_start ? "disabled" : "";
				let status_link = obj.link&&obj.status ? `<span class="ellipsis"> <a target = "_blank" href="${obj.link}">${obj.status || ""}</a></span>` : ""
				html += `<td>
				...
				+ (("\nManufactured Qty : " +  obj.produced_qty) || "")
						...
						<div class="checkbox">
							<label class = "${(obj.status || "").toLowerCase()}" >
								...
								<input type="checkbox"
								class = "stock-entry-controller-${x} stock-entry-controller-${y} stock-entry-controller ${(
				  obj.status || ""
				).toLowerCase()} "
								 autocomplete="off"
								  class="input-with-feedback" 
								data-fieldtype="Check" 
								data-fieldname="${obj.stock_entry || ""}" 
								placeholder="" 
								data-doctype="Stock Entry"
								data-x=${x}
								data-y=${y}
								data-stock-entry = ${obj.stock_entry || ""}																											r
		  ...
		  html += "</table>";
		  //debugger
		  let f = frm.get_field("stock_entries");
		  // frm.get_field("qty_table").$wrapper.append(html)
  
		  frm.refresh_field("stock_entries");
		  let $results = frm.get_field("stock_entries").$wrapper.find(".results");
		  $results.html("");
		  $results.html(html);
		  ...
		  $results.on("change", ".select-all", function (e) {
			...
			$results.find(`.stock-entry-controller`).prop("checked", checked);
		  ...
		  $results.on("change", ".check-x", function (e) {
			...
			let checked = e.target.checked;
			$results.find(`.stock-entry-controller-${x}`).prop("checked", checked);
		  ...
		  $results.on("change", ".check-y", function (e) {
			...
			let checked = e.target.checked;
			$results.find(`.stock-entry-controller-${y}`).prop("checked", checked);
			frm.events.check_axis(frm, $results, checked, "y", y);
		  });
  
		  $results.on("change", ".stock-entry-controller", function (e) {
			let y = e.target.dataset.y;
			let x = e.target.dataset.x;
			let checked = e.target.checked;
	...
	set_totals: function (frm, $results) {
	  ...
	  $.each(frm.doc.values, function (x, xvalue) {
		...
		$results.find(`.x-total-${x}-controller-sf`).prop("value", `${total_f}`);
  
	  });
	  $results.find(`.total-controller`).prop("value", totalxy);
	  $results.find(`.total-controller-sf`).prop("value", `${totalxy_f.toString()}`);
	...
	get_selected: function (frm) {
	  ...
	  $.each(frm.doc.values, function (x, x_value) {
		$.each(x_value, function (y, obj) {
		  if (obj.checked && obj.stock_entry) {
			selected.push(obj);
		  }
		});
	  });
	...
	submit: function (frm) {
	  ...
	  frappe.call({
		method:
		  "smart_manafacturing.smart_manafacturing.doctype.awesome_stock_entry.awesome_stock_entry.submit_all",
		args: {
		  selected: selected, 
		},
		freeze: true,
		...
		callback: function (r) {
		  //frm.refresh()
		  frm.events.get_stock_entries(frm);
		},
	  });
	},
	update_qty: function (frm) {
	  ...
	  frappe.call({
		method:
		  "smart_manafacturing.smart_manafacturing.doctype.awesome_stock_entry.awesome_stock_entry.edit_all",
		args: {
		  selected: selected,
		},
		freeze: true,
