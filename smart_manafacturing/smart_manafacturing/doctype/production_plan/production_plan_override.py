...
class ProductionPlan(Document):
	...
	def get_so_items(self):
		...
		if getattr(self,'filter_item_ref'):
			item_ref_condition = ' and item.item_ref = {0}'.format(frappe.db.escape(self.filter_item_ref))

		items = frappe.db.sql("""select distinct so_item.parent, so_item.item_code, so_item.warehouse,item.item_ref,
			(so_item.qty - so_item.work_order_qty) * so_item.conversion_factor as pending_qty, so_item.description, so_item.name
			from `tabSales Order Item` so_item
   			inner join tabItem item on item.name = so_item.item_code
			where so_item.parent in (%s) and so_item.docstatus = 1 and so_item.qty > so_item.work_order_qty
   			%s
			and exists (select name from `tabBOM` bom where bom.item=so_item.item_code
					and bom.is_active = 1) %s """ % \
			(", ".join(["%s"] * len(so_list)),item_ref_condition, item_condition), tuple(so_list), as_dict=1)
		...
		self.calculate_total_planned_qty()
	@frappe.whitelist()
	def get_current_so_items(self,item_ref,sales_order=''):
    		# Check for empty table or empty rows
		if not self.get("sales_orders") or not self.get_so_mr_list("sales_order", "sales_orders"):
			frappe.throw(_("Please fill the Sales Orders table"), title=_("Sales Orders Required"))

		so_list = [x.sales_order for x in self.get("sales_orders")]
		if sales_order :
			so_list = [sales_order]
		...
		items = frappe.db.sql("""select distinct so_item.parent, so_item.item_code, so_item.warehouse,item.item_ref,
			...
			from `tabSales Order Item` so_item
   			inner join tabItem item on item.name = so_item.item_code
			where so_item.parent in (%s) and so_item.docstatus = 1  and item.item_ref = '%s'
   			and so_item.qty > so_item.work_order_qty
			and exists (select name from `tabBOM` bom where bom.item=so_item.item_code
					and bom.is_active = 1) %s""" % \
			(", ".join(["%s"] * len(so_list)),item_ref, item_condition), tuple(so_list), as_dict=1)
		...
		self.add_current_items(items,sales_order,item_ref)
		self.calculate_total_planned_qty()
	...
	def get_mr_items(self):
		...
		# for i in self.prod_plan_references :
		# 	frappe.msgprint(str(i.__dict__))
		...
		if refs:
			for po_item in self.po_items:
				if po_item.item_code :
					# frappe.msgprint(str('po_item.bom_nooooo'))
					# frappe.msgprint(str(po_item.item_code))
					# frappe.msgprint(str(po_item.bom_no))
	...
	def add_current_items(self, items ,sales_order , item_ref):
	# self.set('po_items', [])
		exist_items = [x for x in getattr(self,'po_items',[]) if x.sales_order == sales_order and x.item_ref == item_ref]
		for data in items:
			if not (data.item_code in exist_items):
				item_details = get_item_details(data.item_code)
				pi = self.append('po_items', {
					'include_exploded_items': 1,
					'warehouse': data.warehouse,
...
def get_sales_orders(self):
	...
	if getattr(self,'filter_item_ref',None):
		item_filter += " and (item.item_ref = '{0}' or so_item.item_ref = '{0}')".format(self.filter_item_ref) 
	sql = f"""
 select distinct so.name, so.transaction_date, so.customer, so.base_grand_total
		from `tabSales Order` so, `tabSales Order Item` so_item , tabItem item
		where so_item.parent = so.name and so_item.item_code = item.name
