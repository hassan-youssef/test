...
@frappe.whitelist()
def get_items_data (doc,from_sales_order=1):
    doc = json.loads(str(doc))
    doc = frappe._dict(doc)
    ...
    if x and y and sales_order and item_ref and from_sales_order:
        ...
        return frappe.db.sql(sql,as_dict=1)
                # frappe.msgprint(sql)
    if x and y and item_ref and  from_sales_order == "false":
            #items = getattr(doc,'po_items',[])
        #if items :
            # frappe.msgprint(str(items))
            # items = json.loads(str(items))
            ...
            #if len(items) > 0:
        sql = """
         select item.name as item_code,item.item_ref,        
            (select attribute_value from `tabItem Variant Attribute` where attribute = '{x}' and parent = item.name limit 1) as x,
            (select attribute_value from `tabItem Variant Attribute` where attribute = '{y}' and parent = item.name limit 1) as y
            from tabItem item
            where item.item_ref = '{item_ref}'
            group by item.name
                """.format(x=x,y=y ,  item_ref = item_ref)
                # frappe.msgprint(sql)
...
def get_item_ref_query(sales_order,item_ref=None):
    ...
    # where name = '{0}' and ifnull(item_ref,'') <> ''
