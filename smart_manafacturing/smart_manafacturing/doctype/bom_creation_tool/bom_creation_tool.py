...
class BOMCreationTool(Document):
	...
	@frappe.whitelist()
	def create_bom(self,selected):
		# selected = json.loads(selected) or []
		# doc = frappe._dict(json.loads(doc))
		doc = self
		print (type(doc))
		if len(selected) == 0 :
			frappe.throw(_('Please Select at least Item'))
		for i in selected :
			# item = frappe._dict(i)
			item_doc = frappe.get_doc("Item" ,i.get('item_code') )
			# frappe.msgprint(item_doc.item_code)	
			# if 1 :
			try :
				bom = frappe.new_doc("BOM")
				bom.company = doc.company
				bom.item = item_doc.item_code
				bom.item_name = item_doc.item_name
				bom.uom = doc.uom
				bom.project = doc.project
				bom.quantity = doc.quantity
				bom.is_active = doc.is_active
				bom.is_default = doc.is_default
				bom.allow_alternative_item = doc.allow_alternative_item
				bom.set_rate_of_sub_assembly_item_based_on_bom = doc.set_rate_of_sub_assembly_item_based_on_bom
				bom.currency = doc.currency
				bom.conversion_rate = doc.conversion_rate
				bom.rm_cost_as_per = doc.rm_cost_as_per
				bom.buying_price_list = doc.buying_price_list
				bom.price_list_currency = doc.price_list_currency
				bom.plc_conversion_rate = doc.plc_conversion_rate
				bom.with_operations = doc.with_operations
				bom.transfer_material_against = doc.transfer_material_against
				bom.routing = doc.routing
				# fetch operations
	
	
				bom.set('operations' , [])
				for row in  getattr(doc,'operations' , []) :
					bom.append('operations',row)
	
	
	
				bom.inspection_required = doc.inspection_required
				bom.quality_inspection_template = doc.quality_inspection_template
				bom.bom_level = doc.bom_level
				...
				bom.set('items' , [])
				items_ss = doc.items or []
				# print('str(type(items_ss)) => ',str(type(items_ss)))
				# print('str(type(items_ss)) => ',items_ss)
				for row in items_ss  :
					# row = frappe._dict(row)
					bom.append('items',row)

				for new_item in i ['items'] :
				...
				bom.set('scrap_items' , [])
				for row in  getattr(doc,'scrap_items' , []) :
					bom.append('scrap_items',row)
	

				bom.operating_cost = doc.operating_cost
				bom.raw_material_cost = doc.raw_material_cost
				bom.scrap_material_cost = doc.scrap_material_cost
				bom.base_operating_cost = doc.base_operating_cost
				bom.base_raw_material_cost = doc.base_raw_material_cost
				bom.base_scrap_material_cost = doc.base_scrap_material_cost
				bom.total_cost = doc.total_cost
				bom.base_total_cost = doc.base_total_cost
				bom.description = item_doc.description
				bom.has_variants = doc.has_variants
	
	
	
				# fetch exploded_items

				bom.set('exploded_items' , [])
				for row in  getattr(doc,'exploded_items' , []) :
					bom.append('exploded_items',row)
	
	
	
				bom.show_in_website = doc.show_in_website
				bom.route = doc.route
				bom.website_image = doc.website_image
				bom.thumbnail = doc.thumbnail
				bom.show_items = doc.show_items
				bom.show_operations = doc.show_operations
				bom.web_long_description = doc.web_long_description
				# # bom.save()
				# print("BOM => ",bom.__dict__)
				# print("BOM => **************************************************************")
				# print("BOM => ",bom.items[0].__dict__)
				bom.submit()
				lnk = get_link_to_form(bom.doctype , bom.name)
				frappe.msgprint(_("BOM {} Created for Item {}").format(lnk,item_doc.item_name))
			except Exception as e :
				frappe.throw(_("Error While Creating BOM for Item {}").format(str(e)))
...
def get_items_by_ref (doc):
	doc = frappe._dict(json.loads(str(doc)))
	x = frappe.db.sql_list("""
				...
				order by itemvalue.idx asc """.format(doc.item_ref)) or []
	...
	items = frappe.db.sql_list(f"""
							select item_code from tabItem where item_ref = '{doc.item_ref}'
	...
	if len (xname) > 0  and len (yname) > 0 :
		for i in items :
			...
			temp = frappe._dict()
			temp.item_code = i
			temp.item_name = itemdoc.item_name
			temp.items = []
			temp.qty = 0
			temp.x = [i.attribute_value for i in itemdoc.attributes if i.attribute == xname[0] ][0].replace(' ','_')
			temp.y = [i.attribute_value for i in itemdoc.attributes if i.attribute == yname[0] ][0].replace(' ','_')
			result.append(temp)
...
@frappe.whitelist()
def get_items_data (doc,items) :
	items = json.loads(str(items))
	doc = frappe._dict(json.loads(str(doc)))
	result = []
	for i in items :
		item = frappe._dict(i)
		if  item.item_code and ( not item.item_name or not item.rate):
			item_doc = frappe.get_doc("Item" , item.item_code)
			item.item_name = item_doc.item_name
			item.qty = item.qty or 1
			details = get_item_details(item.item_code)
			item.uom = details.stock_uom
			details.company = doc.company
			item.rate = get_bom_item_rate(details,doc)
		...
		result.append(item)
			...
			# }
			# details = get_bom_material_detail(args)
	return result
